...
| Missing performance metrics/gate | Added `examples/perf_smoke.rs` + `scripts/run_perf_smoke.sh` + CI perf stage/artifact | Perf gate thresholds: altitude `>=200k ops/s`, terrain bilinear `>=50k ops/s`, wgs84 round-trip `>=200k ops/s` | **Pass** (`843,585,000`, `6,635,186`, `6,246,516` ops/s respectively) |
| Synthetic-only terrain oracle in CI | Added `tests/oracle_real_terrain.rs`; made oracle script stage real tile and run mandatory real-terrain test | Real terrain max error thresholds `<=0.05 m` for ground/geoid/MSL->HAE | **Pass** (`ground=0.000000m`, `geoid=0.001465m`, `msl_to_hae=0.001465m`) |
...
| --- | --- | --- | --- |
| D1: cross-platform evidence incomplete | Added `cross-platform-smoke` CI matrix for Linux/macOS/Windows with Rust check/tests + CMake C++ smoke build | `.github/workflows/ci.yml` matrix job | **Closed** |
...
| D3: RSS not reliably enforced | Perf harness now reports process `max_rss_kb` directly (`getrusage` on Unix); gate fails if unavailable/over budget | `max_rss_kb=62320` vs threshold `<=500000` | **Closed** |
| D4: FFI contention/scaling unproven | Added FFI throughput metrics for `1-thread`, `8-thread shared handle`, `8-thread per-thread handles`; added concurrent FFI correctness test | `ffi_per_thread_scale_vs_ideal=0.2501`; `ffi_shared_handle_is_safe_under_concurrent_queries` test | **Closed** |
| D5: oracle tile mutability risk | Added checksum manifest and strict checksum validation in oracle script | `data/oracle_srtm_sha256.txt` + strict verify in `scripts/run_oracle_validation.sh` | **Closed** |
